...
const session = createAdminSessionGuard({
  prefix: 'df_platform_admin',
  baseUrl: import.meta.env.BASE_URL,
  storage: localStorage,
  location: window.location,
  extraKeys: ['df_shop_admin_token', 'df_shop_admin_refresh_token', 'df_shop_admin_user', 'df_shop_admin_session_id'],
  invalidate: () => window.dispatchEvent(new CustomEvent(ADMIN_SESSION_EXPIRED_EVENT, { detail: { prefix: 'df_platform_admin' } })),
  refreshed: () => window.dispatchEvent(new CustomEvent(ADMIN_SESSION_REFRESHED_EVENT, { detail: { prefix: 'df_platform_admin' } })),
  refresh: async refreshToken => {
    ...
    const { data } = await axios.post<ApiResponse<{ accessToken: string; refreshToken?: string }>>('/api/v1/auth/refresh', { refreshToken }, {
      timeout: 10000,
      headers: { 'Content-Type': 'application/json', 'X-Client-Type': 'platform-admin', 'X-Client-Version': import.meta.env.VITE_APP_VERSION || '0.0.1' }
    })
...
instance.interceptors.request.use((config: SessionRequest) => {
  ...
  config.headers['X-Client-Type'] = 'platform-admin'
