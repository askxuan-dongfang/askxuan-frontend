...
instance.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    const token = localStorage.getItem('df_platform_admin_token')
...
async function refreshAccessToken(): Promise<string | null> {
  const refreshToken = localStorage.getItem('df_platform_admin_refresh_token')
  ...
  localStorage.setItem('df_platform_admin_token', data.data.accessToken)
...
instance.interceptors.response.use(
  (response: AxiosResponse) => {
    ...
    // message-service 可能返回原始 JSON（无 code 字段），直接放行
...
// 类型安全封装：拦截器已解包，方法直接返回 Promise<T>
const client = {
