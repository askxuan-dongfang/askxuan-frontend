...
//  为满足「实时接收新消息通知 / 未读消息实时提示」的需求，本管理器使用 HTTP 轮询模拟实时消息：
//  - 每 5 秒轮询一次未读消息列表（GET /admin/messages/master?isRead=0），以 total 作为未读数
//  - 轮询成功后回调 onDataRefresh，由 MessagesViewModel 刷新消息列表
