...
export function createBooking(data: CreateBookingInput): Promise<Booking> {
  return client.post('/booking', data) as unknown as Promise<Booking>;
}
...
export function getBookings(params?: { status?: string; page?: number; size?: number }): Promise<PagedResult<Booking>> {
  return client.get('/booking', { params }) as unknown as Promise<PagedResult<Booking>>;
}

// 获取预约详情
export function getBooking(id: string): Promise<Booking> {
  return client.get(`/booking/${id}`) as unknown as Promise<Booking>;
}
