...
async function load(){busy.value=true;error.value='';try{const [r,rows]=await Promise.all([client.get<typeof report>('/admin/points/report'),client.get<Product[]|Order[]>(`/admin/points/${tab.value}`,{params:{page:page.value}})]);Object.assign(report,r);if(tab.value==='products')products.value=rows as Product[];else orders.value=rows as Order[]}catch(e){error.value=e instanceof Error?e.message:'加载失败'}finally{busy.value=false}}
...
async function save(){if(!form.name.trim()||!Number.isInteger(form.pointsPrice)||form.pointsPrice<1||!Number.isInteger(form.stock)||form.stock<0){ElMessage.error('请填写商品名称、正整数积分价格和非负整数库存');return}busy.value=true;try{if(form.id)await client.put(`/admin/points/products/${form.id}`,form);else await client.post('/admin/points/products',form);editing.value=false;ElMessage.success('商品已保存');await load()}catch(e){error.value=e instanceof Error?e.message:'操作失败'}finally{busy.value=false}}
async function cancel(o:Order){try{await ElMessageBox.confirm(`取消 ${o.orderNo}，退回 ${o.pointsTotal} 积分并恢复库存？`,'取消兑换')}catch{return}busy.value=true;try{await client.post(`/admin/points/orders/${o.id}/cancel`);await load()}catch(e){error.value=e instanceof Error?e.message:'操作失败'}finally{busy.value=false}}
function openShip(o:Order){shipping.value=o;shipment.carrier='';shipment.trackingNo=''}
async function ship(){if(!shipping.value||!shipment.carrier.trim()||!shipment.trackingNo.trim()){ElMessage.error('请填写物流公司和运单号');return}busy.value=true;try{await client.post(`/admin/points/orders/${shipping.value.id}/ship`,shipment);shipping.value=null;await load()}catch(e){error.value=e instanceof Error?e.message:'操作失败'}finally{busy.value=false}}
function switchTab(){page.value=1;void load()}
onMounted(load)
</script>
<template>
  <div class="points-page">
    <h1>积分商城</h1><p class="intro">独立商品、库存与兑换订单；使用积分兑换，报表独立计算。</p>
    <div class="metrics"><el-card><span>积分商品</span><strong>{{report.productCount}}</strong></el-card><el-card><span>兑换订单</span><strong>{{report.orderCount}}</strong></el-card><el-card><span>已兑换积分（扣除取消）</span><strong>{{report.pointsSpent}}</strong></el-card><el-card><span>待发货</span><strong>{{report.pendingCount}}</strong></el-card></div>
