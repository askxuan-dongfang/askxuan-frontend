...
test('generating, failed retry, insufficient balance and readable report navigation',async({page})=>{
 ...
 paid=true;await page.reload();await expect(page.getByRole('heading',{name:'完整分析',exact:true})).toBeVisible();await page.evaluate(()=>document.fonts.ready);await page.getByText('章节导航',{exact:true}).click();await expect(page.locator('.ai-reading-toc')).toHaveAttribute('open','');await page.getByRole('link',{name:'行动',exact:true}).click();await expect(page.getByRole('heading',{name:'行动',exact:true})).toBeVisible();await page.getByRole('button',{name:'Aa · 放大字号'}).click();await expect(page.locator('.ai-report-prose')).toHaveClass(/large-type/);await expect(page.locator('.ai-report-prose script')).toHaveCount(0);
});
...
for(const width of [320,536]) test(`guided conditional fields and single entry ${width}`,async({page})=>{
 ...
 await page.goto('http://127.0.0.1:5382/c/ai');await expect(page.locator('.ai-topic-grid a')).toHaveCount(7);await expect(page.getByRole('button',{name:/^选择 AI 模型：/})).toHaveCount(1);await expect(page.getByText(/^已选：/)).toHaveCount(0);await expect(page.getByRole('button',{name:'六爻梅花',exact:true})).toHaveCount(0);
