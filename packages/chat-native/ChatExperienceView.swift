...
struct ChatExperienceView: View {
    ...
    var body: some View {
        ZStack {
            Color(red: 28/255, green: 18/255, blue: 16/255).ignoresSafeArea()
            ConversationWebView(conversationID: conversationID, role: role, accountID: accountID,
                ...
                .id(reload)
            if loading { ProgressView("正在打开对话…").tint(.white).foregroundStyle(.white.opacity(0.7)) }
            if let failure {
                VStack(spacing: 18) {
                    ...
                    Button("重新加载") { self.failure = nil; loading = true; reload = UUID() }
                        .buttonStyle(.borderedProminent).tint(Color(red: 0.72, green: 0.32, blue: 0.21))
                    Button("返回") { dismiss() }
                }.padding(30).foregroundStyle(.white)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(Color(red: 28/255, green: 18/255, blue: 16/255))
            }
        }
...
private struct ConversationWebView: UIViewRepresentable {
    ...
    @MainActor final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
        let parent: ConversationWebView; weak var web: WKWebView?
        init(_ parent: ConversationWebView) { self.parent = parent }
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) { parent.onReady() }
